...
class dwm1001_localizer:
    ...
    def moving_average_filter(self, data, window_size):
        """
        Áp dụng bộ lọc trung bình cho dữ liệu

        :param data: Dữ liệu cần lọc
        :param window_size: Kích thước cửa sổ cho bộ lọc trung bình

        :returns: Dữ liệu sau khi đã được lọc
        """
        filtered_data = []
        for i in range(len(data)):
            if i < window_size // 2 or i >= len(data) - window_size // 2:
                # Nếu chỉ số đang xét nằm ngoài phạm vi cửa sổ, giữ nguyên giá trị
                filtered_data.append(data[i])
            else:
                # Tính trung bình của các giá trị trong cửa sổ
                window_values = data[i - window_size // 2: i + window_size // 2 + 1]
    ...
    def pubblishCoordinatesIntoTopics(self, networkDataArray):
        ...
        for network in networkDataArray:
            ...
            if 'AN'.encode() in network:
                ...
                tag_x = float(networkDataArray[networkDataArray.index(network) + 1])
                tag_y = float(networkDataArray[networkDataArray.index(network) + 2])
                tag_z = float(networkDataArray[networkDataArray.index(network) + 3])
                
                # Áp dụng bộ lọc trung bình cho dữ liệu x, y, z
                filtered_tag_x = self.moving_average_filter(filtered_tag_x + [tag_x], 100)[-1]
                filtered_tag_y = self.moving_average_filter(filtered_tag_y + [tag_y], 100)[-1]
                filtered_tag_z = self.moving_average_filter(filtered_tag_z + [tag_z], 100)[-1]
                
                tag = Tag(filtered_tag_x, filtered_tag_y, filtered_tag_z)
