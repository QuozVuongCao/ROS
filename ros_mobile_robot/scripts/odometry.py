...
def subscriber_imu_callback(imu_data):
    ...
    imu_done = True
    imu_yaw = math.radians(imu_data.z)
...
def dead_reckoning(pose):
  ...
  #  delta_theta = imu_yaw - pre_theta
   # theta = pre_theta + 0.5*delta_theta
    pose['x'] += vel['v']*dt*cos(imu_yaw - 0.5*vel['w']*dt)
    pose['y'] += vel['v']*dt*sin(imu_yaw - 0.5*vel['w']*dt)
    pose['yaw'] = imu_yaw 
   # pre_theta = pose['yaw']
...
def main():
    global odom_pub, init_pose_pub, br, previous_time, pose, pre_theta, delta_theta
    rospy.init_node('node_odom')
    odom_pub = rospy.Publisher('/odom', Odometry, queue_size=10)
    start_imu_pub = rospy.Publisher('/start_imu', Bool, queue_size=10)
